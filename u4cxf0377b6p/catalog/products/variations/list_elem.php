    ...
    <div class="content-wrapper">
			<section class="content-header">
				...
				<div class="btn-group wgroup pull-right size13">
                    <a class="btn btn-default btn-xs size14" href="add_elem.php?elem_id=<?=$_GET['elem_id']?>"> <i class="fa fa-plus"></i> <?=$GLOBALS['CPLANG']['ADD_ELEM']?> </a>
                </div>
                <div class="both"></div>
			</section>
						...
						<div class="box">
							...
							<div class="box-body table-responsive no-padding">
								<table class="table table-hover table-striped dataTable">
									...
									<?
									$query = "SELECT id FROM `".$_params['table']."` WHERE section_id = ".$_GET['elem_id']; 
								    $Paginator = pagination($query, $_params['num_page']); 

							        $subcatalog = mysqli_query($db,"SELECT * FROM `".$_params['table']."` WHERE section_id = ".$_GET['elem_id']." ORDER BY `id` DESC LIMIT ".$Paginator['from'].", ".$Paginator['per_page']);
