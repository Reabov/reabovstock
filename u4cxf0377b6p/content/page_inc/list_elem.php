			...
			<section class="content-header">
                ...
                <? if( $User->InGroup( array( 1 ) ) ) { ?>
                    <div class="btn-group wgroup pull-right size13">
                        <a class="btn btn-default btn-xs size14" href="add_inc.php"> <i class="fa fa-plus"></i> <?=$GLOBALS['CPLANG']['ADD_ELEM']?> </a>
                    </div>
                <? } ?>
                <div class="both"></div>
			</section>
							...
							<div class="box-body table-responsive no-padding">
								<table class="table table-hover">
							        ...
							        $ar_pages = mysqli_query($db, "
                                        SELECT * FROM `".$_params['table']."` WHERE `edit_by_user`='1' $fieldToSearch 
                                        ORDER BY `title_".$Admin->lang."` ASC LIMIT ".$Paginator['from'].", ".$Paginator['per_page']);
							        while ( $Elem = mysqli_fetch_array($ar_pages) ) {
							            ?>
							            <tr>
							                <td class="title_line">
