<?
/**
* Class for the basic functions
* 
* @copyright    Copyright (c) 2012, Webmaster Studio, http://www.webmaster.md
* @version      5.0 Date: 2017-07-29
* @link         /ws/include.php
*/
class CMain
{
    ...
    public function GetPageIncData( $inc_id ){
        ...
        $res_page = mysqli_query($db, "SELECT text_".$Lang." FROM `ws_pages_inc` WHERE `id`='".(int)$inc_id."' OR `code`='".$inc_id."'");
        if (!$res_page){
            return 'No data';
        }
    ...
    public function GetDefineSettings(){
        ...
        $res_settings = mysqli_query($db, "SELECT * FROM `ws_settings`");
    ...
    public function GetDefineLangTerms($LNG){
        ...
        $res_terms = mysqli_query($db, "SELECT id, title_".$Lang." AS title, code  FROM `ws_lang_dictionary`");
	...
	{
		global $db;
		$getlang = mysqli_query($db, "SELECT * FROM ws_site_languages WHERE is_default = 1 ");
	...
	{ 
		...
		if( is_dir( $_SERVER['DOCUMENT_ROOT'] . '/desk2021path/' ) ) {
			exit('error code: 802');
		}
