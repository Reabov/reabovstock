...
if( $User->InGroup($_params['access']) ){
	...
	$db_element = mysqli_query($db, "SELECT * FROM `ws_categories_variation` WHERE id='".$_GET['elem_id']."'");
    ...
    <div class="content-wrapper">
			<section class="content-header">
                ...
                <div class="btn-group backbutton">
                	<a class="btn btn-block btn-info btn-xs" href="../list_elem.php?elem_id=<?=$ParentElem['section_id']?>"><?=$GLOBALS['CPLANG']['GO_BACK']?></a>
				</div>
			</section>
						...
						<div class="box">
							...
							<div class="box-body table-responsive no-padding">
								<table class="table table-hover table-striped dataTable">
									...
									<?
									$query = "SELECT id FROM `".$_params['table']."` WHERE var_id = ".$_GET['elem_id']; 
								    $Paginator = pagination($query, $_params['num_page']); 

							        $subcatalog = mysqli_query($db,"SELECT * FROM `".$_params['table']."` WHERE var_id = ".$_GET['elem_id']." ORDER BY `id` DESC LIMIT ".$Paginator['from'].", ".$Paginator['per_page']);
