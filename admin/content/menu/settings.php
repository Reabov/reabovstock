...
function getPagesList($lang, $sid = 0, $level = 0, $selected = 0)
{
    global $db;
    $lv = $level+1;
    $getPages = mysqli_query($db, "SELECT id, title_".$lang." AS title FROM ws_pages WHERE section_id = $sid AND  assoc_table = '' AND edit_by_user = 1 AND id NOT IN(1,2,3) ");
    while( $pg = mysqli_fetch_assoc($getPages) ){
        ?>
        <option value="<?=$pg['id']?>" <?if($selected==$pg['id']){echo "selected";}?>><? for ($i=0; $i <=$level ; $i++) { echo "&bull;&emsp;"; } ?><?=$pg['title']?></option>
        <?
        getPagesList($lang, $pg['id'], $lv);
